...
def main() -> int:
    ...
    # Private documentation names are forbidden in every tracked public file.
    # Split the literals so this validator cannot accidentally match its own
    # source while it performs the repository-wide check.
    validate_local_markdown_links()

    private_markers = ("SON" + "NET", "BIB" + "LIA")
    ...
    private_prose = subprocess.run(
        tuple(_phrase_cmd), cwd=ROOT, text=True, encoding="utf-8", errors="replace",
